...
const SCREENSHOTS_SHEET = "Screenshots";
...
export async function setupSheets() {
  ...
  if (!clientsCheck.data.values) {
    const defaultClients = [
      ...
      ["SummarizeX","LinkedIn,Facebook,Instagram"],
      ["PyaraBaby","Facebook,Instagram"],
      ["Courtesy Honda","Facebook,Instagram,YouTube"],
      ["SheCare","Facebook,Instagram"],
      ["SSW","Facebook,Instagram,LinkedIn"],
      ["Volvo","Facebook,Instagram,YouTube"],
      ["VeriSeek","Facebook,Instagram,LinkedIn"],
      ["Asia Cosmetic","Facebook,Instagram"],
      ["LMK Finance","Facebook,Instagram,LinkedIn"],
      ["Honda","Facebook,Instagram,YouTube"],
      ...
      ["Manthan Work Spaces","Facebook,Instagram,LinkedIn"],
      ["Softradix","LinkedIn,Facebook,Instagram,YouTube"],
      ["Outlander","Facebook,Instagram"],
      ["Kia","Facebook,Instagram,YouTube"],
  ...
  try {
    ...
    if (!sowCheck.data.values) {
      ...
      const sowSeed = [
        ...
        ["8","Asia Cosmetic","PPC + Organic","15","A","Active"],
        ["9","LMK Finance","Organic","15","B","Active"],
        ["10","Honda","PPC + Organic","25","C","Active"],
        ["11","Faith Diagnostic","PPC + Organic","10","D","Active"],
        ["12","Pratha Pre School","PPC + Organic","17","B","Active"],
        ["13","Tress Lounge","Organic","5","B","Inactive"],
        ["14","North International","PPC","4","C","Active"],
        ["15","Manthan Work Spaces","PPC + Organic","12","B","Active"],
  ...
  // ── Rename SOW client names to match Clients sheet ──
  try {
    const SOW_RENAMES = {"1":"SummarizeX","2":"PyaraBaby","3":"Courtesy Honda","4":"SheCare","5":"SSW","6":"Volvo","7":"VeriSeek","8":"Asia Cosmetic","9":"LMK Finance","10":"Honda","11":"Faith Diagnostic","12":"Pratha Pre School","13":"Tress Lounge","14":"North International","15":"Manthan Work Spaces","16":"Softradix","17":"Outlander","18":"Kia","19":"Social Magnet","20":"Body Temple"};
    const sowRenameRes = await sheets.spreadsheets.values.get({spreadsheetId:SPREADSHEET_ID,range:`${SOW_SHEET_NAME}!A2:B1000`});
    const sowRenameRows = sowRenameRes.data.values || [];
    const renameUpdates = [];
    sowRenameRows.forEach((row,i)=>{ const id=String(row[0]); if(SOW_RENAMES[id]&&row[1]!==SOW_RENAMES[id]) renameUpdates.push({range:`${SOW_SHEET_NAME}!B${i+2}`,values:[[SOW_RENAMES[id]]]}); });
    if(renameUpdates.length) await sheets.spreadsheets.values.batchUpdate({spreadsheetId:SPREADSHEET_ID,requestBody:{valueInputOption:"RAW",data:renameUpdates}});
  } catch {}
}
...
export async function uploadScreenshot(fileBase64, fileName, postId, platformName) {
  // Store base64 in dedicated Screenshots sheet — no third party
  const sheets = getSheets();
  const SPREADSHEET_ID = process.env.SPREADSHEET_ID;

  // Ensure Screenshots sheet exists
  const meta = await sheets.spreadsheets.get({ spreadsheetId: SPREADSHEET_ID });
  if (!meta.data.sheets.find(s => s.properties.title === SCREENSHOTS_SHEET)) {
    await sheets.spreadsheets.batchUpdate({
      spreadsheetId: SPREADSHEET_ID,
      requestBody: { requests: [{ addSheet: { properties: { title: SCREENSHOTS_SHEET } } }] }
    });
    await sheets.spreadsheets.values.update({
      spreadsheetId: SPREADSHEET_ID, range: `${SCREENSHOTS_SHEET}!A1`,
      valueInputOption: "RAW",
      requestBody: { values: [["Post ID", "Platform", "File Name", "Uploaded At", "Base64 Data"]] }
    });
  }
  ...
  await sheets.spreadsheets.values.append({
    spreadsheetId: SPREADSHEET_ID,
    range: `${SCREENSHOTS_SHEET}!A2`,
    valueInputOption: "RAW",
    requestBody: { values: [[postId, platformName, fileName || "screenshot.png", now, fileBase64]] }
  });

  // Return a reference key — postId + platform — app uses this to fetch/display
  return `ss:${postId}:${platformName}`;
}

export async function getScreenshot(postId, platformName) {
  const sheets = getSheets();
  const SPREADSHEET_ID = process.env.SPREADSHEET_ID;
  try {
    const res = await sheets.spreadsheets.values.get({
      spreadsheetId: SPREADSHEET_ID,
      range: `${SCREENSHOTS_SHEET}!A2:E5000`,
    });
    const rows = res.data.values || [];
    // Find latest screenshot for this post+platform
    const matches = rows.filter(r => String(r[0]) === String(postId) && r[1] === platformName);
    if (!matches.length) return null;
    const latest = matches[matches.length - 1];
    return { fileName: latest[2], uploadedAt: latest[3], base64: latest[4] };
  } catch { return null; }
}
...
export async function getSOW() {
  ...
  trackerRows.forEach(r => {
    if(!r[0] || !r[1]) return; // skip blank rows (cleared overrides)
...
export async function saveSOWTracker(clientId, month, made) {
  ...
  // made === null means "clear override" — delete the row by blanking it
  if (made === null) {
    if (idx !== -1) {
      const sheetRow = idx + 2;
      await sheets.spreadsheets.values.update({ spreadsheetId: SPREADSHEET_ID, range: `SOW Tracker!A${sheetRow}:C${sheetRow}`, valueInputOption: "RAW", requestBody: { values: [["","",""]] } });
    }
    return;
  }
  if (idx === -1) {
...
export async function updateScreenshotOnly(postId, platformName, screenshotLink) {
  const sheets = getSheets();
  const res = await sheets.spreadsheets.values.get({
    spreadsheetId: SPREADSHEET_ID,
    range: `${SHEET_NAME}!A2:AD2000`,
  ...
  const rows = res.data.values || [];
  const rowIndex = rows.findIndex(r => String(r[COL.ID - 1]) === String(postId));
  if (rowIndex === -1) throw new Error("Post not found");
  const sheetRow = rowIndex + 2;
  const cols = PLAT_COL[platformName];
  if (!cols) throw new Error("Unknown platform");
  const existing = rows[rowIndex][cols.at - 1] || "";
  const timeOnly = existing.split(" | ")[0] || new Date().toLocaleString("en-IN");
  const newAt = timeOnly + " | " + screenshotLink;
  await sheets.spreadsheets.values.update({
    spreadsheetId: SPREADSHEET_ID,
    range: `${SHEET_NAME}!${colLetter(cols.at)}${sheetRow}`,
    valueInputOption: "RAW",
    requestBody: { values: [[newAt]] },
  });
}
